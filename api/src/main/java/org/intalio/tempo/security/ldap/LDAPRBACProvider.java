...
class LDAPRBACProvider implements RBACProvider, LDAPProperties {
    ...
    static Map<String, String> readProperties(String keyRoot, Map source)
        ...
        for (int i = 0; true; i++) {
            ...
            if (front.length() == 0 || back.length() == 0)
                throw new IllegalArgumentException("Format is not reconized! key: " + key
                        + " value: " + value);
            if (result == null)
    ...
    static String getNonNull(String key, Map map) throws IllegalArgumentException {
    ...
    class LDAPRBACQuery implements RBACQuery, LDAPProperties {
        ...
        LDAPRBACQuery(Map map) {
            try {
                _multipleUserOU = readProperties(SECURITY_LDAP_USER_BASE, map).keySet();
            ...
            try {
                _multipleRoleOU = readProperties(SECURITY_LDAP_ROLE_BASE, map).keySet();
            } catch (Exception e) {
         ...
         */
        public String[] authorizedUsers(String role) throws RoleNotFoundException, RBACException,
                RemoteException {
            ...
            try {
                ...
                if (_userRoles != null) {
                    ...
                    _engine.queryFilteredFields(list, _roleBase, _roleId, _roleUsers, _userBase,
                            result);
                }
                return prefix(result);
            } catch (NamingException ne) {
         ...
         */
        public String[] assignedRoles(String user) throws UserNotFoundException, RBACException,
                RemoteException {
            ...
            try {
                ...
                if (_userRoles != null) {
                    ...
                    result = _engine.queryFields(user, userBaseHit, _userId, _userRoles, list);
                    ...
                    while (mrou.hasNext()) {
                        result = _engine.queryRelations(fuser, mrou.next(), _roleUsers, list);
                    }
                } else {
                    // TODO: check this query
                    ...
                    Iterator<String> mrou = _multipleRoleOU.iterator();
                    while (mrou.hasNext()) {
                        result = _engine.queryRelations(user, userBaseHit, _userId, mrou.next(),
                                _roleUsers, checkUser, list);
                    }
                }
         ...
         */
        public String[] authorizedRoles(String user) throws UserNotFoundException, RBACException,
                RemoteException {
            ...
            try {
                ...
                while (mmou.hasNext()) {
                    ...
                    if (_userAllroles != null) {
                        ...
                        result = _engine.queryFields(user, mmou.next(), _userId, _userAllroles,
                                list);
                    } else if (_userRoles != null) {
                        result = _engine.queryFields(user, mmou.next(), _userId, _userRoles, list);
                    } else {
                        boolean checkUser = true;
                        result = _engine.queryRelations(user, mmou.next(), _userId, _roleBase,
                                _roleUsers, checkUser, list);
                    }
         ...
         */
        public String[] descendantRoles(String role) throws RoleNotFoundException, RBACException,
                RemoteException {
         ...
         */
        public String[] ascendantRoles(String role) throws RoleNotFoundException, RBACException,
                RemoteException {
        ...
        public String[] roleOperationsOnObject(String role, String object)
                throws RoleNotFoundException, ObjectNotFoundException, RBACException,
                RemoteException {
        ...
        public String[] userOperationsOnObject(String user, String object)
                throws UserNotFoundException, ObjectNotFoundException, RBACException,
                RemoteException {
            ...
            try {
                ...
                if (_userRoles != null) {
                    ...
                    boolean checkUser = true;
                    found = _engine.queryRelations(user, _userBase, _userId, _roleBase, _roleUsers,
                            checkUser, roles);
                }
                if (found == LDAPQueryEngine.SUBJECT_NOT_FOUND)
         ...
         */
        public Property[] userProperties(String user) throws UserNotFoundException, RBACException,
                RemoteException {
            ...
            try {
                ...
                while (mmou.hasNext() && result.size() == 0) {
                    String userBase = mmou.next();
                    _engine.queryProperties(user, userBase, _userId, _userProp, result);
                }
         ...
         */
        public Property[] roleProperties(String role) throws RoleNotFoundException, RBACException,
                RemoteException {
            ...
            try {
                HashMap<String, Property> result = new HashMap<String, Property>();
                while(mrou.hasNext() && result.size() == 0){
                    String roleBase = mrou.next();
                    _engine.queryProperties(role, roleBase, _roleId, _roleProp, result);
                }
                return (Property[]) result.values().toArray(new Property[result.size()]);
            } catch (NamingException ne) {
                if (LOG.isInfoEnabled())
